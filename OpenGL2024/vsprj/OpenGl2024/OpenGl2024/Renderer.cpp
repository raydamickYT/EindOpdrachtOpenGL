...
void Plane::validate(Renderer* renderer) {
    ...
    if (vertices.empty()) {
        std::cerr << "Error: vertices vector is empty!" << std::endl;
    ...
    // Check if sizes match expected counts
    if (vertices.size() % 3 != 0) {
        std::cerr << "Error: vertices size is not a multiple of 3!" << std::endl;
        throw std::runtime_error("Vertices vector size is incorrect!");
    }

    if (indices.size() % 3 != 0) {
        std::cerr << "Error: indices size is not a multiple of 3!" << std::endl;
        throw std::runtime_error("Indices vector size is incorrect!");
    }
}
...
void Renderer::renderTerrain(unsigned int& planeProgram, WorldInformation& worldInformation, GLuint& terrainIndex, GLuint& terrainVAO)
{
    ...
    glDrawElements(GL_TRIANGLES, terrainIndex, GL_UNSIGNED_INT, 0);
    glUseProgram(0);
}
